...
const POKER = {
    deck: null,
    players: [new Player('You', true), new Player('Dealer')],
    community: [],
    dealerIndex: 1,
    ...
    init: function () {
        ...
        animateDealCards();
        // after dealing, let player view their two cards first; dealer will ask afterwards
        ...
        winners.forEach(w => {
            ...
            if (w.isHuman) {
                if (window.Casino && typeof window.Casino.getCoins === 'function' && typeof window.Casino.setCoins === 'function') {
                    ...
                    // graceful fallback: if Casino not available, attach a no-op or log
...
window.addEventListener('load', () => {
    ...
    if (betBtn) betBtn.addEventListener('click', () => {
        ...
        renderPoker(`已下注 ${val}`);
        // if betting not active, start betting round
        if (!POKER.bettingActive) setTimeout(() => POKER.startBettingRound(), 200);
    });
    const revealBtn = document.getElementById('poker-reveal'); if (revealBtn) revealBtn.addEventListener('click', () => { showdown(); });
    const restartBtn = document.getElementById('poker-restart'); if (restartBtn) restartBtn.addEventListener('click', () => { POKER.init(); });
});
