...
const POKER = {
    ...
    init: function () {
        // 固定為 2 名玩家：You (human) 與 Dealer (AI)
        this.deck = new Deck();
        this.players = [];
    ...
    postBlinds() {
        // For 2 players: dealer posts small blind, other posts big blind
        const sbIdx = this.dealerIndex;
        const bbIdx = (this.dealerIndex + 1) % this.players.length;
        ...
        this.currentBet = bb;
        // turn starts at small blind player (dealer in 2-player)
        this.turnIndex = sbIdx;
        this.bettingActive = true;
    },

    startBettingRound() {
        this.bettingActive = true;
        console.log(`[startBettingRound] Stage: ${this.stage}, Players:`, this.players.map(p => `${p.name}(chips:${p.chips})`).join(', '));

        // Check if betting can continue (at least 1 player with chips)
        const activePlayers = this.players.filter(p => !p.folded && p.chips > 0);
        console.log(`[startBettingRound] Active players with chips:`, activePlayers.map(p => p.name).join(', '));

        if (activePlayers.length === 0) {
            // Everyone is all-in or folded, skip to showdown
            console.log('[startBettingRound] No active players, skipping to showdown');
            this.bettingActive = false;
            this.skipToShowdown();
            return;
        }
        // ensure turnIndex points to first non-folded player with chips
        const maxAttempts = this.players.length;
        let attempts = 0;
        while ((this.players[this.turnIndex].folded || this.players[this.turnIndex].chips === 0) && attempts < maxAttempts) {
            this.turnIndex = (this.turnIndex + 1) % this.players.length;
            attempts++;
        }
        console.log(`[startBettingRound] Turn index set to ${this.turnIndex} (${this.players[this.turnIndex].name})`);
    ...
    processTurn() {
        ...
        // small delay for UX, but use async to avoid stack overflow
    ...
    nextTurn() {
        ...
        // Check if someone has folded (only 1 non-folded player left = instant win)
        const nonFolded = this.players.filter(p => !p.folded);
        if (nonFolded.length === 1) {
            console.log('[nextTurn] Only 1 player left, entering showdown');
            this.bettingActive = false;
            this.stage = 'showdown';
            showdown();
            return;
        }
        // Check if all remaining players are all-in (no one has chips left to bet)
        const allInCheck = this.players.filter(p => !p.folded && p.chips > 0);
        if (allInCheck.length === 0) {
            console.log('[nextTurn] All players all-in, skipping to showdown');
            this.bettingActive = false;
            this.skipToShowdown();
            return;
        }
        ...
        if (this.allBetsSettled()) {
            console.log('[nextTurn] Bets settled, moving to next stage');
            this.bettingActive = false;
            this.endBettingRound();
            return;
        }
        // Use setTimeout to avoid stack overflow from recursive processTurn calls
        setTimeout(() => this.processTurn(), 50);
    },
    ...
    skipToShowdown() {
        // When all remaining players are all-in, automatically deal remaining community cards
        console.log(`[skipToShowdown] Current stage: ${this.stage}, Community cards: ${this.community.length}`);
        if (this.stage === 'preflop') {
            this.community.push(this.deck.draw(), this.deck.draw(), this.deck.draw());
            this.stage = 'flop';
            renderPoker('FLOP - Auto-dealt (all-in)');
            setTimeout(() => this.skipToShowdown(), 800);
        } else if (this.stage === 'flop') {
            this.community.push(this.deck.draw());
            this.stage = 'turn';
            renderPoker('TURN - Auto-dealt (all-in)');
            setTimeout(() => this.skipToShowdown(), 800);
        } else if (this.stage === 'turn') {
            this.community.push(this.deck.draw());
            this.stage = 'river';
            renderPoker('RIVER - Auto-dealt (all-in)');
            setTimeout(() => this.skipToShowdown(), 800);
        } else if (this.stage === 'river') {
            this.stage = 'showdown';
            console.log('[skipToShowdown] Entering showdown...');
            showdown();
        } else {
            console.warn(`[skipToShowdown] Unexpected stage: ${this.stage}`);
        }
    },
...
function showdown() {
    console.log('[showdown] Starting showdown, stage:', POKER.stage, 'community cards:', POKER.community.length);
    const active = POKER.players.filter(p => !p.folded && p.chipsInPlay > 0);
    console.log('[showdown] Active players:', active.map(p => p.name));

    // 檢查每位玩家手牌+社群牌是否足夠比牌
    const minCards = Math.min(...active.map(p => (p.hand.length + POKER.community.length)));
    console.log('[showdown] Min cards available:', minCards);

    // 如果牌數不足（例如直接全押發生在 preflop），自動補齊剩餘社群牌再進行比牌
    if (minCards < 5) {
        console.warn('[showdown] Not enough cards, auto-dealing remaining community cards');
        while (POKER.community.length < 5) {
            const d = POKER.deck.draw();
            if (!d) break; // safety
            POKER.community.push(d);
        }
        renderPoker('自動補齊社群牌，進入結算...');
        // 延遲少許再重新進入 showdown，避免阻塞流程或重入風險
    ...
    const winnersArr = Array.from(winnersSet);
    console.log('[showdown] Winners:', winnersArr, 'Total winnings:', totalWinnings);

    // Show showdown modal with results
    const modal = document.getElementById('showdown-modal');
    const msg = document.getElementById('showdown-msg');
    if (modal && msg) {
        if (totalWinnings > 0) {
            msg.innerHTML = `🎉 恭喜！你贏得 <strong>${totalWinnings}</strong> Coins!<br><br>勝方: <strong>${winnersArr.join(', ')}</strong>`;
            msg.style.color = '#00ff88';
        } else if (winnersArr.length > 0) {
            msg.innerHTML = `勝方: <strong>${winnersArr.join(', ')}</strong><br><br>你本局未獲勝。`;
            msg.style.color = '#ff6b6b';
        } else {
            msg.innerHTML = '本回合無有效勝方。';
            msg.style.color = '#ffd700';
        }
        modal.classList.remove('hidden');
        modal.setAttribute('aria-hidden', 'false');
        console.log('[showdown] Modal displayed');
    } else {
        console.error('[showdown] Modal or message element not found!');
    }
...
window.addEventListener('load', () => {
    // 移除任何重複的 DOM 元素（如多餘的下注欄位）
    const allBetInputs = document.querySelectorAll('#poker-bet');
    if (allBetInputs.length > 1) {
        for (let i = 1; i < allBetInputs.length; i++) {
            allBetInputs[i].remove();
        }
    }
    const allBetBtns = document.querySelectorAll('#poker-bet-btn');
    if (allBetBtns.length > 1) {
        for (let i = 1; i < allBetBtns.length; i++) {
            allBetBtns[i].remove();
        }
    }
    ...
    // 使用委派事件，避免重新 render 後失效
    document.addEventListener('click', (e) => {
        // 下注按鈕
        if (e.target && e.target.id === 'poker-bet-btn') {
        ...
        // 比牌按鈕
        else if (e.target && e.target.id === 'poker-reveal') {
            showdown();
        }
        // 重新開始按鈕
        else if (e.target && e.target.id === 'poker-restart') {
            POKER.init();
        }
        // 結算 modal 關閉按鈕
        else if (e.target && e.target.id === 'showdown-close') {
            const modal = document.getElementById('showdown-modal');
            if (modal) {
