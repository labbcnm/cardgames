...
function renderPoker(msg) {
    ...
    html += '<div class="players">';
        ...
        html += `<div class="action-area" style="margin-top:12px">`;
        if (isHumanTurn) {
            const toCall = Math.max(0, POKER.currentBet - human.chipsInPlay);
            html += `<div class="to-call" style="margin-bottom:8px">需跟注：<strong>${toCall}</strong></div>`;
            html += `<div class="buttons" style="display:flex;gap:8px;align-items:center">`;
            html += `<button id="p-fold">棄牌</button>`;
            if (toCall === 0) html += `<button id="p-check">過牌</button>`;
            else html += `<button id="p-call">跟注 ${toCall}</button>`;
            html += ` 加注：<input id="p-raise-val" type="number" min="1" value="${Math.max(POKER.bigBlind,10)}" style="width:90px"> <button id="p-raise">加注</button>`;
            html += `<button id="p-allin">全押</button>`;
            html += `</div>`;
        } else {
            html += `<div class="info">現在不是你的回合</div>`;
        }
        html += `</div>`;
