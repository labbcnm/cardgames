...
function showdown() {
    ...
    pots.forEach(pot => {
        ...
        // 過濾掉無法比牌的玩家
        const validResults = results.filter(r => r.best && r.best.score);
        if (validResults.length === 0) return;
        let max = validResults[0].best.score; let winners = [validResults[0].player];
        for (let i = 1; i < validResults.length; i++) {
            const cmp = compareScores(validResults[i].best.score, max);
            if (cmp > 0) { max = validResults[i].best.score; winners = [validResults[i].player]; }
            else if (cmp === 0) { winners.push(validResults[i].player); }
        }
