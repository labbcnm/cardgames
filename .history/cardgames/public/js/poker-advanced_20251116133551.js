...
function showdown() {
    ...
    // Update round-result area with details
    const rr = document.getElementById('round-result');
    if (rr) {
        if (totalWinnings > 0) {
            rr.className = 'round-result win';
            rr.innerHTML = `🎉 你贏得 <strong>${totalWinnings}</strong> Coins! <br>勝方: <strong>${winnersArr.join(', ')}</strong>`;
        } else if (winnersArr.length > 0) {
            rr.className = 'round-result';
            rr.innerHTML = `勝方: <strong>${winnersArr.join(', ')}</strong>。你沒有獲勝。`;
        } else {
            rr.className = 'round-result';
            rr.innerText = '本回合無有效勝方。';
        }
    }
    renderPoker('Showdown complete.');
    // small chip animation when player wins
    if (totalWinnings > 0) {
        const wallet = document.getElementById('wallet');
        const potEl = document.querySelector('.community-row') || document.getElementById('game-area');
        if (wallet && potEl) animateChips(0, potEl, wallet);
    }
}
