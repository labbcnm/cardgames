...
function showdown() {
    ...
    let msg = 'Showdown complete.';
    if (winnersArr.length > 0) msg += ' Winner(s): ' + winnersArr.join(', ');
    if (totalWinnings > 0) msg = `🎉 你贏得 ${totalWinnings} Coins! ` + (winnersArr.length > 0 ? 'Winner(s): ' + winnersArr.join(', ') : '');
    renderPoker(msg);
}
