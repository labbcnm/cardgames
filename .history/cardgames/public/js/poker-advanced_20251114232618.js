...
function compareScores(a, b) {
    // lexicographic compare of arrays/numbers
    if (a === b) return 0;
    // if not arrays, compare as numbers
    const isArrA = Array.isArray(a), isArrB = Array.isArray(b);
    if (!isArrA && !isArrB) return (a > b) ? 1 : (a < b ? -1 : 0);
    if (isArrA && !isArrB) return 1;
    if (!isArrA && isArrB) return -1;
    // both arrays
